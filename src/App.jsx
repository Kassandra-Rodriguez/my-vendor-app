...
const CATEGORIES = ["All", "MiniBits", "CardKits", "Keychains", "Mini Figures", "Custom"];
...
const GlobalStyle = () => (
  <style>{`
    ...
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .bg-orb1 { width: 260px; height: 260px; background: #7c3aed; top: -60px; left: -60px; animation-delay: 0s; }
    .bg-orb2 { width: 200px; height: 200px; background: #f97316; bottom: 20%; right: -40px; animation-delay: -3s; }
    .bg-orb3 { width: 150px; height: 150px; background: #06b6d4; top: 40%; left: 30%; animation-delay: -5s; }
    @keyframes orb-drift {
    ...
    .light .header { background: rgba(245,243,255,0.85); }
    .header-title {
    ...
    .btn::after { content: ''; position: absolute; inset: 0; border-radius: inherit; opacity: 0; transition: opacity 0.2s; background: rgba(255,255,255,0.1); }
    .btn:hover::after { opacity: 1; }
    .btn:active { transform: scale(0.97); }
    .btn:disabled { opacity: 0.5; cursor: not-allowed; }
    .btn-primary { background: linear-gradient(135deg, #7c3aed, #c084fc); color: white; padding: 14px 24px; box-shadow: 0 4px 20px rgba(124,58,237,0.35); }
    .btn-primary:hover { box-shadow: 0 4px 30px rgba(124,58,237,0.55); transform: translateY(-1px); }
    .btn-orange { background: linear-gradient(135deg, #ea580c, #f97316); color: white; padding: 14px 24px; box-shadow: 0 4px 20px rgba(249,115,22,0.35); }
    .btn-orange:hover { box-shadow: 0 4px 30px rgba(249,115,22,0.55); transform: translateY(-1px); }
    .btn-ghost { background: var(--surface); border: 1px solid var(--border); color: var(--text2); padding: 12px 20px; }
    .btn-ghost:hover { border-color: var(--accent); color: var(--accent); }
    .btn-danger { background: linear-gradient(135deg, #dc2626, #f87171); color: white; padding: 12px 20px; }
    .btn-full { width: 100%; }

    .input-group { margin-bottom: 14px; }
    .input-label { display: block; font-size: 12px; font-weight: 600; color: var(--text3); text-transform: uppercase; letter-spacing: 0.08em; margin-bottom: 6px; }
    .input {
      width: 100%;
      background: var(--surface);
      border: 1px solid var(--border);
    ...
    .modal-title { font-family: 'Syne', sans-serif; font-weight: 700; font-size: 20px; margin-bottom: 20px; }
    ...
    .product-card::before { content: ''; position: absolute; top: 0; left: 0; right: 0; height: 2px; background: linear-gradient(90deg, var(--accent), var(--accent2)); opacity: 0; transition: opacity 0.2s; }
    ...
    .product-card .qty-badge { position: absolute; top: 10px; right: 10px; width: 24px; height: 24px; border-radius: 8px; background: var(--accent); color: white; font-size: 12px; font-weight: 700; display: flex; align-items: center; justify-content: center; }
    .product-card .cat { font-size: 11px; color: var(--text3); margin-top: 4px; }

    .stats-row { display: flex; gap: 10px; padding: 16px 20px; }
    .stat-card { flex: 1; background: var(--surface); border: 1px solid var(--border); border-radius: var(--radius); padding: 14px 12px; text-align: center; }
    .stat-value { font-family: 'Syne', sans-serif; font-weight: 800; font-size: 20px; line-height: 1; }
    ...
    .hero-card::after { content: ''; position: absolute; top: -30px; right: -30px; width: 120px; height: 120px; border-radius: 50%; background: rgba(192,132,252,0.2); }
    .hero-amount { font-family: 'Syne', sans-serif; font-weight: 800; font-size: 38px; color: white; line-height: 1; }
    ...
    .search-wrap { position: relative; margin: 0 20px 12px; }
    .search-icon { position: absolute; left: 12px; top: 50%; transform: translateY(-50%); font-size: 16px; color: var(--text3); }
    .search-input { padding-left: 38px !important; }

    .cat-pills { display: flex; gap: 8px; padding: 0 20px 12px; overflow-x: auto; scrollbar-width: none; }
    .cat-pills::-webkit-scrollbar { display: none; }
    .cat-pill { flex-shrink: 0; padding: 6px 14px; border-radius: 20px; font-size: 13px; font-weight: 500; cursor: pointer; border: 1px solid var(--border); background: var(--surface); color: var(--text2); transition: all 0.2s; white-space: nowrap; }
    ...
    .qty-controls { display: flex; align-items: center; gap: 16px; margin: 20px 0; }
    .qty-btn { width: 52px; height: 52px; border-radius: 16px; border: 1px solid var(--border); background: var(--surface); font-size: 22px; color: var(--text); cursor: pointer; transition: all 0.15s; display: flex; align-items: center; justify-content: center; }
    .qty-btn:hover { background: var(--accent); border-color: var(--accent); color: white; transform: scale(1.05); }
    .qty-btn:active { transform: scale(0.92); }
    .qty-display { flex: 1; text-align: center; font-family: 'Syne', sans-serif; font-weight: 800; font-size: 40px; color: var(--text); }
    ...
    .ripple-el { position: absolute; border-radius: 50%; width: 40px; height: 40px; background: var(--accent); pointer-events: none; animation: ripple 0.5s ease-out forwards; }
    ...
    .error-msg { color: var(--danger); font-size: 13px; padding: 10px 14px; background: rgba(248,113,113,0.1); border-radius: 10px; margin-bottom: 14px; border: 1px solid rgba(248,113,113,0.2); }
    ...
    .live-dot { width: 8px; height: 8px; border-radius: 50%; background: var(--accent3); animation: live-pulse 1.5s ease-in-out infinite; display: inline-block; margin-right: 6px; }
    @keyframes live-pulse { 0%, 100% { box-shadow: 0 0 0 0 rgba(52,211,153,0.5); } 50% { box-shadow: 0 0 0 6px rgba(52,211,153,0); } }

    .event-item { background: var(--surface); border: 1px solid var(--border); border-radius: var(--radius); padding: 16px; margin-bottom: 10px; cursor: pointer; transition: all 0.2s; }
    ...
    .spinner { width: 32px; height: 32px; border: 3px solid var(--border); border-top-color: var(--accent); border-radius: 50%; animation: spin 0.7s linear infinite; margin: 0 auto; }
...
const Particles = () => {
  ...
  return <>{particles.map(p => <div key={p.id} className="particle" style={{ left: p.left, animationDelay: p.delay, animationDuration: p.duration }} />)}</>;
};
...
const Toast = ({ msg }) => <div className={`toast ${msg ? "show" : ""}`}>{msg}</div>;
...
const dbToProduct = (row) => ({
  ...
  category: row.category, cost: row.cost, active: row.active,
});
const dbToEvent = (row) => ({
  id: row.id, date: row.date, location: row.location, notes: row.notes,
  lineItems: row.line_items || {},
  squareTotal: row.square_total || "",
  cashAppTotal: row.cash_app_total || "",
  vendorFee: row.vendor_fee || "",
  otherExpenses: row.other_expenses || "",
  cashRevenue: row.cash_revenue || "",
  status: row.status || "draft",
});
const eventToDb = (ev, userId) => ({
  id: ev.id, user_id: userId, date: ev.date, location: ev.location,
  notes: ev.notes || "", line_items: ev.lineItems || {},
  square_total: ev.squareTotal || "", cash_app_total: ev.cashAppTotal || "",
  vendor_fee: ev.vendorFee || "", other_expenses: ev.otherExpenses || "",
  cash_revenue: ev.cashRevenue || "", status: ev.status || "draft",
});
const productToDb = (p, userId) => ({
  id: p.id, user_id: userId, name: p.name, price: p.price,
  category: p.category || "", cost: p.cost || 0, active: p.active !== false,
});

// ─── Main App ─────────────────────────────────────────────────────────────────
export default function App() {
  const [theme, setTheme] = useState(() => load("vst_theme") || "dark");
  ...
  useEffect(() => {
    if (!session) { setProducts([]); setEvents([]); return; }
    loadData();
  }, [session]);
  ...
  const loadData = async () => {
    setDataLoading(true);
    try {
      const [{ data: prods }, { data: evts }] = await Promise.all([
  ...
  // Sync active event back to list
  useEffect(() => {
    if (!activeEvent) return;
    setEvents(prev => prev.map(e => e.id === activeEvent.id ? activeEvent : e));
  }, [activeEvent]);

  useEffect(() => { save("vst_theme", theme); }, [theme]);

  const toggleTheme = () => setTheme(t => t === "dark" ? "light" : "dark");

  const upsertProduct = async (p) => {
    const row = productToDb(p, session.user.id);
  ...
  const createEvent = async (ev) => {
    const newEv = { ...ev, id: uid(), lineItems: {}, squareTotal: "", cashAppTotal: "", vendorFee: "", otherExpenses: "", cashRevenue: "", status: "draft" };
    const { error } = await supabase.from("events").insert(eventToDb(newEv, session.user.id));
  ...
  const tapProduct = (productId) => {
    ...
    const updatedEvent = { ...activeEvent, lineItems: updated, lastAction: { productId, qty: prev.qty } };
    setActiveEvent(updatedEvent);
    // Save to Supabase (debounced via state sync)
    supabase.from("events").update({ line_items: updated }).eq("id", activeEvent.id);
  };
  ...
  const finalizeEvent = async (data) => {
    const finalized = { ...activeEvent, ...data, status: "done" };
    const { error } = await supabase.from("events").update(eventToDb(finalized, session.user.id)).eq("id", finalized.id);
  ...
  const computeRevenue = (ev) => {
    ...
    return { cash, square, cashApp, gross, expenses, net: gross - expenses, items };
  };

  const exportCSV = (ev) => {
    const rows = [["Product", "Qty", "Unit Price", "Total"]];
    Object.entries(ev.lineItems || {}).forEach(([_, li]) => rows.push([li.name, li.qty, li.price.toFixed(2), (li.qty * li.price).toFixed(2)]));
    ...
    a.download = `${ev.location || "event"}-${ev.date}.csv`;
    a.click();
  };
  ...
  if (authLoading) return (
    <div className={`app ${theme === "light" ? "light" : ""}`}>
      ...
      <div className="screen" style={{ display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div><div className="spinner" /><div style={{ textAlign: "center", color: "var(--text3)", fontSize: 13, marginTop: 16 }}>Loading...</div></div>
      </div>
    </div>
  );
  ...
  const screens = {
    home: <HomeScreen events={events} computeRevenue={computeRevenue} onOpenEvent={(ev) => { setActiveEvent(ev); setTab("live"); }} onCreateEvent={() => setModal({ type: "createEvent" })} loading={dataLoading} />,
    live: activeEvent
  ...
  return (
    <div className={`app ${theme === "light" ? "light" : ""}`}>
      ...
      <BgLayer />
      <Header
        title={tab === "home" ? "VendorTrack" : tab === "live" ? "Live Sales" : tab === "products" ? "Products" : "History"}
        onTheme={toggleTheme}
        theme={theme}
      ...
      {tab === "live" && activeEvent && <button className="undo-fab" onClick={undoLast} title="Undo last tap">↩</button>}

      {modal?.type === "product" && <ProductModal product={modal.data} onSave={upsertProduct} onClose={() => setModal(null)} />}
      ...
      <div className="login-wrap">
        ...
        <div style={{ width: "100%", maxWidth: 340 }}>
          <div className="auth-toggle">
            <button className={`auth-toggle-btn ${mode === "login" ? "active" : ""}`} onClick={() => { setMode("login"); setError(""); setSuccess(""); }}>Log In</button>
            <button className={`auth-toggle-btn ${mode === "signup" ? "active" : ""}`} onClick={() => { setMode("signup"); setError(""); setSuccess(""); }}>Sign Up</button>
          </div>
          {error && <div className="error-msg">⚠️ {error}</div>}
          {success && <div style={{ color: "var(--accent3)", fontSize: 13, padding: "10px 14px", background: "rgba(52,211,153,0.1)", borderRadius: 10, marginBottom: 14, border: "1px solid rgba(52,211,153,0.2)" }}>✓ {success}</div>}
          <div className="input-group">
            <label className="input-label">Email</label>
            <input className="input" type="email" placeholder="you@example.com" value={email} onChange={e => setEmail(e.target.value)} onKeyDown={e => e.key === "Enter" && handle()} autoFocus />
          </div>
          <div className="input-group">
          ...
          <button className="btn btn-primary btn-full" style={{ height: 52 }} onClick={handle} disabled={loading}>
...
// ─── Header ───────────────────────────────────────────────────────────────────
function Header({ title, onTheme, theme, userEmail, onLogout }) {
    ...
    <div className="header">
      ...
      <div className="header-actions">
        ...
        <div style={{ position: "relative" }}>
          <div
            style={{ width: 32, height: 32, borderRadius: "50%", background: "linear-gradient(135deg,#7c3aed,#f97316)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, color: "white", cursor: "pointer" }}
            onClick={() => setShowMenu(m => !m)}
          >
            {userEmail?.[0]?.toUpperCase() || "?"}
          </div>
          {showMenu && (
            <div style={{ position: "absolute", right: 0, top: 40, background: "var(--surface2)", border: "1px solid var(--border)", borderRadius: 14, padding: 8, minWidth: 180, zIndex: 20, boxShadow: "0 8px 30px rgba(0,0,0,0.4)" }}>
              <div style={{ fontSize: 12, color: "var(--text3)", padding: "6px 10px 10px" }}>{userEmail}</div>
              <button
                style={{ width: "100%", padding: "10px 12px", borderRadius: 10, border: "none", background: "transparent", color: "var(--danger)", cursor: "pointer", textAlign: "left", fontSize: 14, fontWeight: 600 }}
...
// ─── Home Screen ──────────────────────────────────────────────────────────────
function HomeScreen({ events, computeRevenue, onOpenEvent, onCreateEvent, loading }) {
  const recent = [...events].reverse().slice(0, 10);
  const active = events.find(e => e.status === "draft");
  const done = events.filter(e => e.status === "done");
  const totalRevenue = done.reduce((s, e) => s + computeRevenue(e).gross, 0);
  ...
  return (
    <div>
      <div className="hero-card">
        ...
        <div className="hero-meta">{done.length} completed events</div>
      </div>
        ...
        <div className="section">
          <div style={{ background: "linear-gradient(135deg, #064e3b, #065f46)", borderRadius: "var(--radius)", padding: 16, cursor: "pointer", border: "1px solid rgba(52,211,153,0.2)" }} onClick={() => onOpenEvent(active)}>
            <div className="flex items-center justify-between">
              <div>
                <div style={{ fontSize: 12, color: "rgba(255,255,255,0.6)", marginBottom: 4 }}><span className="live-dot" />ACTIVE EVENT</div>
      ...
      {events.length > 0 && (
        <div className="stats-row">
          <div className="stat-card stat-orange"><div className="stat-value">{events.length}</div><div className="stat-label">Events</div></div>
          <div className="stat-card stat-green"><div className="stat-value">{fmt(done.reduce((s, e) => s + computeRevenue(e).net, 0))}</div><div className="stat-label">Net Profit</div></div>
          <div className="stat-card stat-purple"><div className="stat-value">{done.length > 0 ? fmt(totalRevenue / done.length) : "$0"}</div><div className="stat-label">Avg/Event</div></div>
        </div>
      )}

      <div className="section mb-3">
        <button className="btn btn-orange btn-full" style={{ height: 56 }} onClick={onCreateEvent}>+ Create New Event</button>
      </div>

      <div className="section">
        <div className="section-header"><div className="section-title">Recent Events</div></div>
        {loading ? <div style={{ textAlign: "center", padding: 40 }}><div className="spinner" /></div>
          : recent.length === 0 ? (
            <div className="empty"><div className="empty-icon">🛒</div><div className="empty-text">No events yet. Create your first event!</div></div>
          ) : recent.map(ev => {
            const r = computeRevenue(ev);
            return (
              <div key={ev.id} className="event-item" onClick={() => ev.status === "draft" ? onOpenEvent(ev) : null}>
                <div className="flex items-center justify-between">
                  <div><div className="event-name">{ev.location}</div><div className="event-date">{dateStr(ev.date)}</div></div>
                  <span className={`badge ${ev.status === "done" ? "badge-green" : "badge-orange"}`}>{ev.status === "done" ? "Done" : "Live"}</span>
                </div>
...
function LiveScreen({ event, products, onTap, onUndo, onAdjust, computeRevenue, onEndDay }) {
  ...
  const filtered = products.filter(p => p.active !== false).filter(p => {
  ...
  return (
    <div>
      <div className="stats-row" style={{ paddingTop: 12 }}>
        <div className="stat-card stat-green"><div className="stat-value">{fmt(r.cash)}</div><div className="stat-label">💵 Cash Total</div></div>
        <div className="stat-card stat-purple"><div className="stat-value">{r.items}</div><div className="stat-label">Items Sold</div></div>
        <div className="stat-card stat-orange"><div className="stat-value">{Object.keys(event.lineItems || {}).length}</div><div className="stat-label">Products</div></div>
      </div>
      ...
      <div className="cat-pills">
        {CATEGORIES.map(c => <div key={c} className={`cat-pill ${cat === c ? "active" : ""}`} onClick={() => setCat(c)}>{c}</div>)}
      </div>

      <div className="section">
        {filtered.length === 0 ? (
          <div className="empty"><div className="empty-icon">📦</div><div className="empty-text">No products match. Add products in the Products tab.</div></div>
        ) : (
  ...
  return (
    <div>
      ...
      <div className="section">
        ...
        {active.length === 0 && <div className="empty"><div className="empty-icon">🍰</div><div className="empty-text">No products yet. Add your first product!</div></div>}
    ...
    <div className="card" style={{ marginBottom: 10, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
      <div style={{ flex: 1 }}>
        ...
        <div style={{ display: "flex", gap: 8, marginTop: 4, flexWrap: "wrap" }}>
          ...
          {p.cost > 0 && <span className="text-xs text-muted">cost {fmt(p.cost)}</span>}
        </div>
      </div>
          ...
          <div key={ev.id} className="event-item" onClick={() => onOpen(ev)}>
            <div className="flex items-center justify-between">
              ...
              <div style={{ textAlign: "right" }}>
                ...
                <div className="text-xs" style={{ color: r.net >= 0 ? "var(--accent3)" : "var(--danger)" }}>{r.net >= 0 ? "+" : ""}{fmt(r.net)} net</div>
              </div>
            </div>
            <div className="event-totals">
              <span className="text-xs text-muted">💵 {fmt(r.cash)}</span>
              {parseFloat(ev.squareTotal) > 0 && <span className="text-xs text-muted">◼ {fmt(ev.squareTotal)}</span>}
              {parseFloat(ev.cashAppTotal) > 0 && <span className="text-xs text-muted">💰 {fmt(ev.cashAppTotal)}</span>}
              <span className="text-xs text-muted">{r.items} items</span>
            </div>
          </div>
        );
    ...
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal">
        ...
        <div className="input-group">
          ...
          <input className="input" value={form.name} onChange={e => set("name", e.target.value)} placeholder="e.g. Mushroom MiniBit" autoFocus />
        </div>
        <div className="input-group">
        ...
        <div style={{ display: "flex", gap: 10 }}>
          ...
          <button className="btn btn-primary" style={{ flex: 2 }} disabled={!valid} onClick={() => { onSave({ ...form, price: parseFloat(form.price), cost: parseFloat(form.cost) || 0 }); onClose(); }}>Save Product</button>
        </div>
      </div>
    </div>
  );
      ...
      <div className="modal">
        ...
        {active.length > 0 && (
          <div style={{ fontSize: 13, color: "var(--text3)", marginBottom: 16, padding: "10px 14px", background: "var(--surface)", borderRadius: 10, border: "1px solid var(--border)" }}>
            📦 {active.length} active product{active.length > 1 ? "s" : ""} will be available
          </div>
        )}
        <div style={{ display: "flex", gap: 10 }}>
          <button className="btn btn-ghost" style={{ flex: 1 }} onClick={onClose}>Cancel</button>
          <button className="btn btn-orange" style={{ flex: 2 }} disabled={!valid} onClick={() => { onCreate(form); onClose(); }}>Start Event 🚀</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal">
        ...
        <div style={{ textAlign: "center", fontSize: 13, color: "var(--text3)", marginBottom: 20 }}>Subtotal: {fmt(qty * product.price)}</div>
        <div style={{ display: "flex", gap: 10 }}>
          <button className="btn btn-ghost" style={{ flex: 1 }} onClick={onClose}>Cancel</button>
  ...
  return (
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal">
        ...
        <div className="input-group"><label className="input-label">Square Total ($)</label><input className="input" type="number" min="0" step="0.01" value={form.squareTotal} onChange={e => set("squareTotal", e.target.value)} placeholder="0.00" /></div>
        <div className="input-group"><label className="input-label">Cash App Total ($)</label><input className="input" type="number" min="0" step="0.01" value={form.cashAppTotal} onChange={e => set("cashAppTotal", e.target.value)} placeholder="0.00" /></div>
        <div className="input-group"><label className="input-label">Cash Revenue Override (optional)</label><input className="input" type="number" min="0" step="0.01" value={form.cashRevenue} onChange={e => set("cashRevenue", e.target.value)} /></div>
        <div className="divider" />
        <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 10, color: "var(--text2)" }}>Expenses</div>
        <div className="input-group"><label className="input-label">Vendor Fee ($)</label><input className="input" type="number" min="0" step="0.01" value={form.vendorFee} onChange={e => set("vendorFee", e.target.value)} placeholder="0.00" /></div>
        <div className="input-group"><label className="input-label">Other (travel, supplies, etc.)</label><input className="input" type="number" min="0" step="0.01" value={form.otherExpenses} onChange={e => set("otherExpenses", e.target.value)} placeholder="0.00" /></div>
        <div className="divider" />
        <div style={{ background: "linear-gradient(135deg, rgba(124,58,237,0.15), rgba(249,115,22,0.1))", borderRadius: 14, padding: 16, marginBottom: 20, border: "1px solid rgba(192,132,252,0.2)" }}>
          <div className="summary-row"><span className="summary-label">Gross Revenue</span><span className="summary-val color-accent2">{fmt(gross)}</span></div>
          <div className="summary-row"><span className="summary-label">Total Expenses</span><span className="summary-val text-danger">−{fmt(expenses)}</span></div>
          <div className="summary-row"><span className="summary-label" style={{ fontWeight: 700 }}>Net Profit</span><span className="summary-val" style={{ fontSize: 22, color: net >= 0 ? "var(--accent3)" : "var(--danger)" }}>{fmt(net)}</span></div>
        </div>
        <div style={{ display: "flex", gap: 10 }}>
    ...
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal">
        ...
        <div style={{ background: "var(--surface)", borderRadius: 14, padding: 14, marginBottom: 16, border: "1px solid var(--border)" }}>
          <div className="summary-row"><span className="summary-label">💵 Cash Sales</span><span className="summary-val">{fmt(r.cash)}</span></div>
          {parseFloat(event.squareTotal) > 0 && <div className="summary-row"><span className="summary-label">◼ Square</span><span className="summary-val">{fmt(event.squareTotal)}</span></div>}
          {parseFloat(event.cashAppTotal) > 0 && <div className="summary-row"><span className="summary-label">💰 Cash App</span><span className="summary-val">{fmt(event.cashAppTotal)}</span></div>}
          <div className="summary-row"><span className="summary-label">Gross Revenue</span><span className="summary-val color-accent2">{fmt(r.gross)}</span></div>
          {r.expenses > 0 && <div className="summary-row"><span className="summary-label">Expenses</span><span className="summary-val text-danger">−{fmt(r.expenses)}</span></div>}
          <div className="summary-row"><span className="summary-label" style={{ fontWeight: 700 }}>Net Profit</span><span className="summary-val" style={{ fontSize: 20, color: r.net >= 0 ? "var(--accent3)" : "var(--danger)" }}>{fmt(r.net)}</span></div>
        </div>
        <div style={{ fontSize: 14, fontWeight: 600, marginBottom: 10 }}>Sales Breakdown</div>
        {sorted.length === 0 ? <div className="text-muted text-sm">No items recorded.</div>
          : sorted.map(([id, li]) => (
            <div key={id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 0", borderBottom: "1px solid var(--border)" }}>
              <div><div style={{ fontWeight: 500 }}>{li.name}</div><div className="text-xs text-muted">{li.qty} × {fmt(li.price)}</div></div>
